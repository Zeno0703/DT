...
ieval2 (Tell s t) answers = (s:qstns, res)
    where (qstns, res) = ieval2 t answers
ieval2 (Ask q ts) [] = ([q], Nothing)
ieval2 (Ask q ts) (b:as)   =  (q: qstns, res)
    where (qstns, res) = ieval2 (ts b) as
...
ieval7 (Tell s t) = do  putStrLn s
                        ieval7 t
ieval7 (Ask q t) = do   putStrLn q
                        ans <- getLine
                        let action
                                    | ans == "yes" = ieval7 (t True)
